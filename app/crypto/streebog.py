# 1) сюда вставь PI, TAU, A_MATRIX_ROWS (64 строк по 8 байт), C[12]
#    из RFC 6986, §6.2..6.5
from typing import List

PI: List[int] = [252, 238, 221, 17, 207, 110, 49, 22, 251, 196, 250, 218, 35, 197, 4, 77, 233, 119, 240, 219, 147,
                 ...
                 81,
                 234, 200, 72, 171, 242, 42, 104, 162, 253, 58, 206, 204, 181, 112, 14, 86, 8, 12, 118, 18, 191, 114,
                 19,
...
TAU: List[int] = [
    ...
    27, 35, 43, 51, 59, 4, 12, 20, 28, 36, 44, 52, 60, 5, 13, 21, 29, 37, 45, 53, 61, 6, 14, 22, 30, 38, 46, 54,
...
A_ROWS = [int(row, 16) if isinstance(row, str) else row for row in A_ROWS]

C = [bytes.fromhex(c) if isinstance(c, str) else c for c in C]
...
def expand_keys(K0: bytes) -> list[bytes]:
    # K[0] = K1, ..., K[12] = K13 по стандарту
    K = [None] * 13
    K[0] = K0
    for i in range(1, 13):
        # C[0] = C[1] в RFC, ..., C[11] = C[12]
        K[i] = LPS(xor512(K[i - 1], C[i - 1]))
    return K


def E(K0: bytes, m: bytes) -> bytes:
    K = expand_keys(K0)  # длина 13: K[0]=K1, ..., K[12]=K13
    state = m
    # X[K1],LPS, ..., X[K12],LPS
    for i in range(12):          # 0..11 -> K1..K12
        state = xor512(state, K[i])
        state = LPS(state)
    # финальный X[K13]
    state = xor512(state, K[12])
    return state


def g_N(h: bytes, N: bytes, m: bytes) -> bytes:
    K0 = LPS(xor512(h, N))
    return xor512(E(K0, m), xor512(h, m))
